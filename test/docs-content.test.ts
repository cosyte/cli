 ...
 *
 * `resolve` points a snippet's `import ... from "@cosyte/cli"` at this repo's source, the same way the
...
docSnippetSuite({
  ...
  resolve: (specifier) =>
    specifier === "@cosyte/cli" ? join(import.meta.dirname, "..", "src", "index.ts") : undefined,
});
