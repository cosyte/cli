 ...
 * **format-specific arbitraries** (the `Cli` generators below).
 *
 * This file is the intended shape for every `@cosyte/*` parser (see `@cosyte/hl7`'s
 * `test/property/`). While `@cosyte/cli` is still a scaffold:
 *
 ...
 *   - the **round-trip** invariant is `it.todo` until the serializer (`serializeCli` /
...
import { FATAL_CODES, WARNING_CODES, parseCli, type ParsedCli } from "../../src/index.js";
 ...
 * just arbitrary strings; replace it with a generator that emits real CLI quirks (truncated
 ...
 */
function specCleanCli(): fc.Arbitrary<ParsedCli> {
  return fc.constant({ value: {}, warnings: [] } satisfies ParsedCli);
}

describe("cli conformance (archetype invariants)", () => {
  it("is lenient — arbitrary input never throws a non-fatal, and every warning has a known code", () => {
    lenientNeverThrowsProperty({
      ...
      getWarnings: (parsed) => (parsed as ParsedCli).warnings,
  ...
  // TODO: flip `it.todo` -> `it` once a serializer (`serializeCli` / `result.toString()`)
