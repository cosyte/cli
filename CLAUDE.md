...
**`@cosyte/cli`** — a developer-focused CLI parser + utility library for Node.js/TypeScript,
...
**North star (the archetype):** a developer can parse a real-world, vendor-quirky CLI message
...
- **Scaffolded from the shared `@cosyte/*` parser template.** Pre-alpha `0.0.x`, not yet published to
  npm. `src/index.ts` carries archetype **stubs** (`parseCli`, `WARNING_CODES`, `FATAL_CODES`)
