...
`CLAUDE.md` was cut to fit the declared line ceiling for an always-read agent doc, and the way it was
cut is the way this repository has cut one before: every rule stayed there as a one-line imperative,
and the sentences that explained it moved here. This section is those sentences, **verbatim**, section
by section and in the order they stood. Nothing was deleted, softened or summarised on the way over;
only the section headings were re-titled so they nest under this one.

Read it the way the rest of this file is read: **the compressed line in `CLAUDE.md` binds, and the
paragraph here is why.** If a rule up there ever reads as arbitrary, its unabbreviated form is below,
and **"I did not read the reason" is still not a licence to discount the rule.**
### The two-file gate, before the trim
...
### The redact delegation and its refusals
...
### The published package and the FHIR hole, before the trim
...
### Hard runtime deps, before the trim
...
### The docs sidebar, before the trim
...
### The pre-commit PHI scanner, before the trim
...
### The em-dash brand gate, before the trim
...
### Branch protection, before the trim
...
### Engineering Guardrails, before the trim
...
### Standing disciplines, before the trim
