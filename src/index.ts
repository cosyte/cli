/**
 * Public entry point for the `@cosyte/cli` package.
 *
...
/**
 * The result of parsing a CLI payload.
 *
...
/**
 * Options for {@link parseCli}.
 *
 * The parser is **lenient by default** (Postel's Law): vendor quirks become {@link CliWarning}s
...
/**
 * Parse a CLI payload into a {@link ParsedCli}.
 *
 * **Lenient by default** — real-world, vendor-quirky input parses rather than throws, emitting
 * {@link CliWarning}s instead (Postel's Law). The complementary serializer (added in a later
 * phase) always emits spec-clean output. Only unrecoverable structural corruption throws (a Tier-3
 * {@link FatalCode}).
 *
 * @param raw - The raw CLI payload (`string` for now; `Buffer`/`Uint8Array` support is added
 *   alongside the real parser).
 * @param options - Parse options; see {@link ParseCliOptions}. Lenient unless `strict` is set.
 ...
 * ```
 */
export function parseCli(raw: string, options: ParseCliOptions = {}): ParsedCli {
 ...
 * @example
 * ```ts
 * import { WARNING_CODES } from "@cosyte/cli";
 *
 ...
 * @example
 * ```ts
 * import { FATAL_CODES } from "@cosyte/cli";
 *
 * try {
 *   parseCli(raw);
 * } catch (err) {
